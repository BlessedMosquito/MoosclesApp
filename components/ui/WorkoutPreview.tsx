...
export default function WorkoutPreview({
  ...
  async function toggleExercise(exercise: ReturnGetExercisesData) {
    ...
    try {
      ...
      setSetsByExercise(c => ({ ...c, [exerciseId]: setData as ReturnGetSetsData[] }));
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : 'Could not load sets.');
    } finally {
  ...
  function renderMetricCard(label: string, value: string) {
    return (
      <div key={label} style={{
        ...
        <p style={{ margin: `${s(4, scale)}px 0 0`, fontWeight: 700, fontSize: s(fontSizes.body, scale) }}>
          {value}
        </p>
      </div>
    );
    ...
    >
        <div style={{
          ...
          marginBottom: s(4, scale),
            ...
            <h2 style={{ margin: `${s(6, scale)}px 0 0`, fontSize: s(fontSizes.heading2, scale) }}>
                {workout.name}
            </h2>
            <button
                    type="button"
      ...
      <div style={{ marginTop: s(14, scale), display: 'flex', flexDirection: 'column', gap: s(8, scale) }}>
        {isLoading ? (
          <div style={{ display: 'flex', justifyContent: 'center', padding: s(8, scale) }}>
            <LoadingCircle />
          </div>
            ...
            exercises.map((exercise, index) => {
              ...
              return (
                <div key={exercise.id} style={{
                  ...
                  <button type="button" onClick={() => toggleExercise(exercise)} style={{
                    width: '100%',
                    border: 'none',
                  ...
                  {isExpanded && (
                    <div style={{ marginTop: s(14, scale), display: 'flex', flexDirection: 'column', gap: s(10, scale) }}>
                      {isLoadingSets ? (
                        <div style={{ display: 'flex', justifyContent: 'center', padding: s(8, scale) }}>
